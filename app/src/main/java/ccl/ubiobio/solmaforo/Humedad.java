...
public class Humedad extends AppCompatActivity {
    ...
    public void humedadActual (String fecha){
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, WS_URL, null , new Response.Listener<JSONObject>() {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                int humActual= leer.getInt("humedadActual",0);
                if(humActual!=0) {
                    gaugeHumedad.speedTo(humActual);
                }else{
                    //gaugeHumedad.speedTo(leer.getInt("humedadActual",0));
                    Toast.makeText(Humedad.this,"Ocurrio un error en la obtencion de datos actuales",Toast.LENGTH_SHORT).show();
    ...
    public void humedadPromedio (String fecha){
        ...
        JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, WS_URL, null , new Response.Listener<JSONObject>() {
            ...
            public void onErrorResponse(VolleyError error) {
                Log.d("LOG_WS",error.toString());
                int humProm= leer.getInt("humedadPromedio",0);
                if(humProm!=0){
                    gaugeHumedadProm.speedTo(humProm);
                }
                else {
                    Toast.makeText(Humedad.this, "Ocurrio un error en la obtencion de datos del promedio", Toast.LENGTH_SHORT).show();
                }
            }
        });
        requestQueue.add(request);
